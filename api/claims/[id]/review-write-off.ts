...
/**
 * POST /api/claims/[id]/review-write-off
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try { await sql`ALTER TABLE claims ADD COLUMN IF NOT EXISTS write_off_denied_at timestamptz` } catch {}
    try { await sql`ALTER TABLE claims ADD COLUMN IF NOT EXISTS write_off_denied_by uuid` } catch {}
    try { await sql`ALTER TABLE claims ADD COLUMN IF NOT EXISTS write_off_denied_note text` } catch {}
    ...
    const [existing] = await sql`
      SELECT id, status, write_off_pending, write_off_reason, write_off_note
      FROM claims
      WHERE id = ${claimId}::uuid AND practice_id = ${practiceId}::uuid
      LIMIT 1
    `
    if (!existing) return res.status(404).json({ error: 'Claim not found' })
    if (!existing.write_off_pending) return res.status(409).json({ error: 'No pending write-off request on this claim.' })
    ...
    if (approved) {
      const [updated] = await sql`
        UPDATE claims SET
          status                = 'written_off',
          written_off_at        = NOW(),
          written_off_by        = ${ownerId}::uuid,
          write_off_pending     = FALSE,
          updated_at            = NOW()
        WHERE id = ${claimId}::uuid AND practice_id = ${practiceId}::uuid
        RETURNING *
      `
      return res.status(200).json({ action: 'approved', claim: updated })
    }
    ...
    const [updated] = await sql`
      ...
      RETURNING *
    `
    return res.status(200).json({ action: 'denied', claim: updated })
  } catch (e: any) {
    console.error('claims/[id]/review-write-off error:', e)
