...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    ...
    const [row] = await sql`
      ...
      VALUES (${familyId}::uuid, ${childIds}::uuid[], ${b.visit_type}, ${b.preferred_provider ?? null}, ${b.zone ?? null}, ${b.state ?? null}, ${b.preferred_date}::date, ${b.preferred_time}, ${b.status ?? 'pending'}, ${b.confirmed_provider_id ?? null}, ${b.reference_code}, ${b.convenience_fee ?? null}, ${b.notes ?? null}, ${practiceId}::uuid)
      RETURNING *`
    return res.json(row)
  }
