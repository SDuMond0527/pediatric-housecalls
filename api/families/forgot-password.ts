...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log('forgot-password(family): sending reset link to', normalizedEmail, '| RESEND_KEY set:', !!RESEND_API_KEY, '| URL:', resetUrl)

  if (!RESEND_API_KEY) {
    console.error('forgot-password(family): RESEND_API_KEY not set — cannot send email')
  ...
  try {
    ...
    if (!r.ok) {
      console.error('forgot-password(family): Resend error', r.status, JSON.stringify(body))
    } else {
      console.log('forgot-password(family): Resend accepted, id=', body.id)
    }
  } catch (e: any) {
    console.error('forgot-password(family): fetch failed', e?.message)
  }
