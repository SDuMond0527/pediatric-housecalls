...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const [days, overrides, zoneRestrictions, timeBlocks, visitTypes] = await Promise.all([
      sql`SELECT * FROM availability WHERE provider_id = ${providerId}::uuid AND practice_id = ${practiceId} ORDER BY day_of_week`,
      sql`SELECT * FROM availability_overrides WHERE provider_id = ${providerId}::uuid AND practice_id = ${practiceId} ORDER BY date`,
      sql`SELECT * FROM zone_restrictions WHERE provider_id = ${providerId}::uuid AND practice_id = ${practiceId}`,
      sql`SELECT * FROM time_blocks WHERE provider_id = ${providerId}::uuid AND practice_id = ${practiceId}`,
      sql`SELECT * FROM visit_type_availability WHERE provider_id = ${providerId}::uuid AND practice_id = ${practiceId}`,
    ])
    ...
    const results = await Promise.all(days.map(async (d) => {
      if (d.id) {
        const [row] = await sql`
          ...
          WHERE id=${d.id}::uuid AND practice_id = ${practiceId} RETURNING *`
        return row
      } else {
        const [row] = await sql`
          INSERT INTO availability (provider_id, day_of_week, is_active, start_time, end_time, practice_id)
          VALUES (${providerId}::uuid, ${d.day_of_week}, ${d.is_active}, ${d.start_time}, ${d.end_time}, ${practiceId})
