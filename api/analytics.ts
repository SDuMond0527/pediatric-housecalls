...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const [appointments, bookingRequests, waitlistEntries, familyProfiles, providers, broadcasts, onCallShifts] = await Promise.all([
    ...
    sql`SELECT id, status, state, family_id, converted_provider_id, removal_reason, parent_response, created_at, parent_response_at FROM waitlist_entries WHERE practice_id = ${practiceId}::uuid`,
    ...
    sql`SELECT provider_id, date::text, state, start_time, end_time FROM on_call_schedule WHERE practice_id = ${practiceId}::uuid`,
  ])

  res.json({ appointments, bookingRequests, waitlistEntries, familyProfiles, providers, broadcasts, onCallShifts })
}
