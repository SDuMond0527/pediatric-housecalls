  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[13px] text-[#1A1A2E] mt-1">Vaccines administered by our practice</p>
      </div>

      {loading && (
        <div className="text-center py-16 text-[#1A1A2E] text-[14px]">Loading vaccine records…</div>
      )}

      {!loading && notes.length === 0 && (
        <div className="text-center py-16">
          <Shield size={32} className="text-[#E8E8E4] mx-auto mb-3" />
          <div className="text-[14px] text-[#1A1A2E]">No vaccine records on file.</div>
          <div className="text-[13px] text-[#1A1A2E] mt-1">Vaccines will appear here after they are administered and the note is signed.</div>
        </div>
      )}
                        ...
                        <table className="w-full text-[13px] min-w-[480px]">
                          <thead>
                            <tr className="border-b border-[#E8E8E4] bg-[#FAFAF8]">
                              <th className="text-left px-4 py-2.5 text-[11px] text-[#1A1A2E] font-medium">Vaccine</th>
                              <th className="text-left px-4 py-2.5 text-[11px] text-[#1A1A2E] font-medium">Lot #</th>
                              <th className="text-left px-4 py-2.5 text-[11px] text-[#1A1A2E] font-medium">Dose</th>
                              <th className="text-left px-4 py-2.5 text-[11px] text-[#1A1A2E] font-medium">Route / Site</th>
                            </tr>
                          </thead>
                          <tbody>
                            {(note.vaccine_administrations ?? []).map((v, i) => (
                              <tr key={i} className={i > 0 ? 'border-t border-[#F0F0EC]' : ''}>
                                <td className="px-4 py-3 font-medium text-[#1A1A2E]">
                                  {v.vaccine_name}
                                  {v.manufacturer && (
                                    <div className="text-[11px] text-[#1A1A2E] font-normal">{v.manufacturer}</div>
                                  )}
                                </td>
