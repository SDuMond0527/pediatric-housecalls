...
export function ForgotPassword() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      await fetch('/api/providers/forgot-password', {
        method: 'POST',
            ...
            {sent ? (
              <div className="text-center">
                ...
                <p className="text-[13px] text-[#555] leading-relaxed mb-5">
                  If <strong>{email}</strong> is registered as a provider, you'll receive a password reset link shortly.
                </p>
                <Link to="/login" className="text-[13px] text-[#7F77DD] font-medium hover:underline">
                  Back to sign in
                </Link>
              </div>
            ) : (
